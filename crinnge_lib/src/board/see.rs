use crate::{
    board::{lookup_bishop_moves, lookup_rook_moves},
    types::*,
};
...
impl Board {
    pub fn see_beats_threshold(&self, mv: Move, threshold: i16) -> bool {
        debug_assert!(self.piece_on(mv.from()).is_some());
        let Some(piece) = self.piece_on(mv.from()) else {
            return false;
        };

        // correct for ep capture
        let mut value = if piece == Pawn && mv.to().bitboard() == self.ep_mask {
            ...
            self.piece_on(mv.to())
                ...
                .unwrap_or(0)
                + SEE_PIECE_VALUES[mv.promo().unwrap_or(Pawn)]
        ...
        let recapture = if let Some(promo) = mv.promo() {
            promo
        } else {
            piece
        };
        value -= SEE_PIECE_VALUES[recapture];
        ...
        let mut occupied = self.all_pieces() ^ mv.from().bitboard();
        // remove ep pawn
        if piece == Pawn && mv.to().bitboard() == self.ep_mask {
            occupied &= !((self.ep_mask << 8) | (self.ep_mask >> 8));
        }
        ...
        let mut color = !self.player;

        loop {
            // remove used pieces from attackers
            attackers &= occupied;

            let current_pieces = self.occupied[color];
            let other_pieces = self.occupied[!color];
            ...
            if value >= 0 {
                // TODO: account for promotions after the first move
            ...
            if matches!(piece, Pawn | Bishop | Queen) {
                attackers |= lookup_bishop_moves(mv.to(), occupied) & bishops;
            }

            if matches!(piece, Rook | Queen) {
                attackers |= lookup_rook_moves(mv.to(), occupied) & rooks;
            }
        }
...
mod tests {
    ...
    fn test_see() -> Result<(), Box<dyn Error>> {
        ...
        for (fen, move_, score) in test_cases {
            ...
            // }
            if fen == "2r5/1P4pk/p2p1b1p/5b1n/BB3p2/2R2p2/P1P2P2/4RK2 w - - 0 1" {
                dbg!();
            }
